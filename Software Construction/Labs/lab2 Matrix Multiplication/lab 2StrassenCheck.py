...
#Functions to check for Power of 2 and make power of 2. Use for Strassen's Algo
...
#Check whether array is valid for Strassen's Algorithm or not
...
def arrayCheck(a):
    ...
    if rows!=cols:
        if rows > cols:
            ...
            rows=cols
    power2=checkPowerofTwo(rows)
    if power2==False:
        rows=makePowerofTwo(rows)
    return resizeArray(a,rows)


#This works on np arrays, so first convert it into such array
